...
themis_status_t themis_secure_cell_encrypt_seal(const uint8_t* master_key,
  ...
  themis_status_t st = themis_auth_sym_encrypt_message(master_key, master_key_length, message, message_length, user_context, user_context_length, NULL, &ctx_length_, NULL, &msg_length_);
	...
	if(st!=(themis_status_t)THEMIS_BUFFER_TOO_SMALL) {
		syslog(LOG_CRIT, "THEMIS LOG: themis_secure_cell_encrypt_seal: log12");
	}

	THEMIS_STATUS_CHECK(st, (themis_status_t)THEMIS_BUFFER_TOO_SMALL);
    ...
    syslog(LOG_CRIT, "THEMIS LOG: themis_secure_cell_encrypt_seal: log3: master_key_length = %zu, user_context_length = %zu, message_length = %zu, ctx_length_ = %zu, msg_length_ = %zu",
           master_key_length, user_context_length, message_length, ctx_length_, msg_length_);
  ...
  syslog(LOG_CRIT, "THEMIS LOG: themis_secure_cell_encrypt_seal: log4: master_key_length = %zu, user_context_length = %zu, message_length = %zu, encrypted_message_length = %zu, ctx_length_ = %zu, msg_length_ = %zu",
...
themis_status_t themis_secure_cell_decrypt_seal(const uint8_t* master_key,
  ...
  syslog(LOG_CRIT, "THEMIS LOG: themis_secure_cell_decrypt_seal: log2: master_key_length = %zu, user_context_length = %zu, message_length = %zu, encrypted_message_length = %zu, ctx_length_ = %zu, msg_length_ = %zu",
  	master_key_length, user_context_length, msg_length_, encrypted_message_length, ctx_length_, msg_length_);
